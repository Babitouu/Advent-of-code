...
class Jour_1:
    ...
    def rotation(nombre:int,rotation_demande:str)->int:
        """
        Effectue une rotation sur le cadran en fonction de la demande.
        """
        if rotation_demande[0] == "L":
            nombre -= int(rotation_demande[1:])%100 # Rotation à gauche
        else:
            nombre += int(rotation_demande[1:])%100 # Rotation à droite
        return nombre
    
    def resoudre(self,rotations:list)->int:
        """
        Résout le problème en analysant les rotations et en comptant le nombre de fois où le cadran pointe sur 0.
        """
        numero = 50 # Le cadran commence à 50
